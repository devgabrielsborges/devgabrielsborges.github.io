...
async function fetchGitHubRepos() {
  try {
    ...
    console.log('Latest 3 repositories:', filteredRepos.map(r => r.name));
...
async function transformRepoData(repos) {
  ...
  for (const repo of repos) {
    ...
    const description = repo.description || 'Projeto em desenvolvimento ativo no GitHub.';
...
async function updateProjectsJson(repos) {
  try {
    ...
    await writeFile(PROJECTS_JSON_PATH, JSON.stringify(projectsData, null, 2), 'utf-8');
...
async function main() {
  try {
    ...
    console.log(`🔑 GitHub token: ${GITHUB_TOKEN ? 'provided' : 'not provided (rate limits may apply)'}`);
    ...
    console.log('💡 The Vue component will automatically load the updated data.');
  } catch (error) {
